...
def handle_client_connection(client_socket,address): 
    ...
    headerformat='!BLL'
    try:
        while True: # Ler a estrutura a partir de um Protocolo!
            request = client_socket.recv(struct.calcsize(headerformat)) # Cabecalho
            pktheader=struct.unpack(headerformat,request)
            version,size,order=pktheader
            request = client_socket.recv(size) # Temos o tamanho!!!
